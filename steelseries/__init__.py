...
def send_event_data_interpolated(
    ...
    if not silent:
        if successful_sends == steps + 1:
            ...
            print(f"[INFO]  Animation completed with {successful_sends}/{steps + 1} successful sends")
...
def remove_game(gamename, silent=False):
    data = { "game": gamename }

    try:
        response = requests.post(f"http://{sse_address}/remove_game", json=data)
        response.raise_for_status()
        if not silent:
            print("[OK] Game removed.")
    except Exception as e:
        if not silent:
            print(f"[Error] Failed to remove game: {e}")

def remove_event(gamename, eventname, silent=False):
    data = { "game": gamename, "event": eventname }

    try:
        response = requests.post(f"http://{sse_address}/remove_game_event", json=data)
        response.raise_for_status()
        if not silent:
            print(f"[OK] Event '{eventname}' removed.")
    except Exception as e:
        if not silent:
            print(f"[Error] Failed to remove event '{eventname}': {e}")
